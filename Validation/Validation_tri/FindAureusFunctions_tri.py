...
def tri_bacteria (input_image):
    thresh_tri = filters.threshold_triangle
    tri_image = (input_image > thresh_tri(input_image)).astype(np.uint8)
    return (tri_image)
...
def FindBacteriaAndNoBacteria_tri(input_image_list, file_metadata):
    '''
...
file_metadata : TYPE dict
    ...
    for imageno in tqdm(range(0,len(input_image_list))):
        ...
        tri_image = tri_bacteria(input_image)
        contours_avaliable = FindingContours(tri_image)
        
        bac_pixel_coordinates = GetPixelWiseBacteriaCoordinates(tri_image)
