...
def OTSU_bacteria (input_image):
    ret, otsu_image = cv2.threshold(input_image, 0, np.max(input_image), cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    return (otsu_image)
...
def FindBacteriaAndNoBacteria_otsu(input_image_list, file_metadata):
    '''
...
file_metadata : TYPE dict
    ...
    for imageno in tqdm(range(0,len(input_image_list))):
        ...
        otsu_image = OTSU_bacteria(input_image)
        contours_avaliable = FindingContours(otsu_image)
        
        bac_pixel_coordinates = GetPixelWiseBacteriaCoordinates(otsu_image)
...
def FindAureus_otsu():
    ...
    faureus = FindBacteriaAndNoBacteria_otsu(channelimagelist, inputimagefilemetadata)
    if faureus[0] != []:
